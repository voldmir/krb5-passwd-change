...
#define RETURN_ON_ERROR(message, code) \
    ...
    { \
        ...
        PyErr_Format(PyExc_BaseException, "%s: %s", message, error); \
...
_k5_set_password_error(krb5_data *result_code_string, krb5_data *result_string)
{
    ...
    p1 = malloc((result_code_string->length)*4);
    if (p1 == NULL)
    {
        PyErr_NoMemory();
        return;
    ...
    p2 = malloc((result_string->length)*4);
    if (p1 == NULL)
    {
        PyErr_NoMemory();
        return;
    ...
    PyErr_Format(PyExc_BaseException, "%s%s%s", p1, (*p1 && *p2) ? ": " : "", p2);
...
k5_cc_default(PyObject *self, PyObject *args)
{
    ...
    if (name == NULL)
    {
        PyErr_Format(PyExc_BaseException, "krb5_cc_default() returned NULL");
        return NULL;
    }
